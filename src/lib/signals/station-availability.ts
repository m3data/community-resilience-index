 ...
 *
 * Data sources:
 *   - WA FuelWatch (daily, diesel + petrol)
 *   - NSW FuelCheck (daily, diesel + petrol)
 *
 * Station counts are derived dynamically from snapshot data (uniqueStationCount).
 * Do not hardcode counts — they change as sources update.
 *
...
export function fetchStationAvailability(): Signal | null {
  ...
  if (files.length < 2) {
    return {
      ...
      propagatesTo:
        "Reduced station availability increases travel distance to fuel and concentrates demand at remaining outlets",
      components,
    };
  }
